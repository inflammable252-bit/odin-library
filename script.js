...
function updateCards() {
	...
	allReadButtons.forEach((item) => {
		item.addEventListener("click", (e) => {
			...
			readTextInCard.className =
				readTextInCard.className === "read" ? "notRead" : "read";
			readTextInCard.textContent =
				readTextInCard.textContent === "Read" ? "Not read" : "Read";
		});
	});
	...
	console.log(shelves);
	console.log(
		`# of shelves: ${numberOfShelves} / total books: ${myLibrary.length}`,
	);
...
function validateInputs() {
	if (title.validity.valueMissing) {
		title.setCustomValidity("Your book must have a name!");
	} else title.setCustomValidity("");
	if (author.validity.valueMissing) {
		author.setCustomValidity("Your book must have an author!");
	} else author.setCustomValidity("");
	if (pages.validity.valueMissing) {
		pages.setCustomValidity("Your book must have pages!");
...
function addDemoBooks() {
	...
	addBookToLibrary(
		...
		"638",
		"not read",
		"Confusing and experimental narrative. Reconsidering.",
	);
	...
	addBookToLibrary(
		...
		"800",
		"read",
		"Reminder: review analyses for poems on marked pages.",
	);
}
